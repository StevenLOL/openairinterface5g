...
uint32_t  dlsch_decoding_2thread0(dlsch_td_params* tdp)
{

	PHY_VARS_UE *phy_vars_ue   		= tdp->UE;
	int eNB_id         				= tdp->eNB_id;
	int harq_pid      				= tdp->harq_pid;
	int llr8_flag     				= tdp->llr8_flag;
	UE_rxtx_proc_t *proc    		= tdp->proc;
	int subframe      				= proc->subframe_rx;
	LTE_UE_DLSCH_t *dlsch 			= phy_vars_ue->dlsch[phy_vars_ue->current_thread_id[subframe]][eNB_id];
	LTE_DL_UE_HARQ_t *harq_process  = dlsch->harq_processes[harq_pid];
	short *dlsch_llr 				= phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id[subframe]][eNB_id]->llr;
...
uint32_t  dlsch_decoding_2thread(PHY_VARS_UE *phy_vars_ue,
                         short *dlsch_llr,
                         LTE_DL_FRAME_PARMS *frame_parms,
                         LTE_UE_DLSCH_t *dlsch,
                         LTE_DL_UE_HARQ_t *harq_process,
                         ...
                         uint8_t llr8_flag)
{
  ...
  uint32_t r,r_offset=0,Kr,Kr_bytes,err_flag=0;
  uint8_t crc_type;
  UE_proc_t *proc = &phy_vars_ue->proc;
  int Cby2;
#ifdef DEBUG_DLSCH_DECODING
  uint16_t i;
#endif
