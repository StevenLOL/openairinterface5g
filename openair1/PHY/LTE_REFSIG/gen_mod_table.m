...
% First Amplitude for QAM64 (\f$ 2^15 \times 8/\sqrt{170}\f$)
QAM256_n1 = 20105;
% Second Amplitude for QAM64 (\f$ 2^15 \times 4/\sqrt{170}\f$)
QAM256_n2 = 10052;
% Third Amplitude for QAM64 (\f$ 2^15 \times 2/\sqrt{170}\f$)
QAM256_n3 = 5026;
% Third Amplitude for QAM64 (\f$ 2^15 \times 1/\sqrt{170}\f$)
QAM256_n4 = 2513;
...
%% QAM256 *******************************************************************************************
  for a=-1:2:1 
	...
	qam256_table(index+1) = -a*(QAM256_n1 + b*(QAM256_n2 + (c*(QAM256_n3+QAM256_n4))));
      end
    end
  end	
end
...
%%qam64_table2(b0*32+b1*16+b2*8+b3*4+b4*2+b5*1+1) = qam64_table(b0*4+b2*2+b4*1+1) + 1j*qam64_table(b1*4+b3*2+b5*1+1);
qam256_table2(b0*128+b1*64+b2*32+b3*16+b4*8+b5*4+b6*2+b7*1+1) = qam256_table(b0*8+b2*4+b4*2+b6*1+1) + 1j*qam64_table(b1*8+b3*4+b5*2+b7*1+1);
end
end
end
end
...
table = round(K * [0; qpsk_table2(:); qam16_table2(:); qam64_table2(:); qam256_table2(:);psync_table(:)]); 
