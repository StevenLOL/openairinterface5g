...
MCS = [8 8]; % MCS for the 2 users, currently it is assumed that mcs(2)=mcs(1)
j = sqrt(-1);
amp = 1/32;
XFORMS = 1;
...
edata0 = DlschEncoding(data0,simparms,simparms.codeword(1)); % user 1
edata1 = DlschEncoding(data1,simparms,simparms.codeword(2)); % user 2
 ...
 for iSNR=1:length(simparms.snr)
 	...
 	for n=1:simparms.n_frames
        ...
        % reorder for processing
 		...
 		%% Compute Scaling
 		avg = DlschChannelLevelPrec_TM56(H_fxp_t,pmi_ext,simparms);
 		simparms.log2_maxh = max(double(Log2Approx(avg))-13,0);		
        ...
        %% Inner receiver loop
 		...
 		for symbol = 4:14
			...
			[ymf0(idxs:idxe,:)...
				...
				Hmag0(idxs:idxe,:)...
				Hmagb0(idxs:idxe,:)]...
				= DlschChannelCompensation_TM56(y_fxp_t,H_fxp_t,pmi_ext,simparms,simparms.codeword(1),symbol-1);
