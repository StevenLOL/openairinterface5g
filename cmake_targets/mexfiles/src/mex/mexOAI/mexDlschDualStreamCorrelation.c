...
void mexDlschDualStreamCorrelation( int nlhs, mxArray *plhs[],
                ...
                )
{
  ...
  unsigned int nb_re_per_symbol = 300, nb_re_per_frame = 4200;
  unsigned char output_shift = 0, symbol;
  PHY_VARS_UE *ue;
  mxArray *tmp;
  ...
  tmp = mxGetField(prhs[2],0,"nb_antennas_tx_eNB");
  if (tmp == NULL) {
    mexErrMsgTxt("Non-existing field 'nb_antennas_tx_eNB' in input argument 3.");
  } else {
  ...
  rho10 = (int*) mxCalloc(nb_re_per_frame*ue->frame_parms.nb_antennas_rx*ue->frame_parms.nb_antennas_tx, sizeof(int));
  ...
  dl_ch_estimates_ext_i[3] = &Heff1[3*nb_re_per_frame];
    ...
    /* Algo */
  dlsch_dual_stream_correlation(&ue->frame_parms,
                                symbol,
  ...
  memcpy(rho10_out,&dl_ch_rho_ext[0][symbol*ue->frame_parms.N_RB_DL*12],nb_re_per_symbol<<2);
  memcpy(&rho10_out[nb_re_per_symbol],&dl_ch_rho_ext[1][symbol*ue->frame_parms.N_RB_DL*12],nb_re_per_symbol<<2);
  memcpy(&rho10_out[2*nb_re_per_symbol],&dl_ch_rho_ext[2][symbol*ue->frame_parms.N_RB_DL*12],nb_re_per_symbol<<2);
  memcpy(&rho10_out[3*nb_re_per_symbol],&dl_ch_rho_ext[3][symbol*ue->frame_parms.N_RB_DL*12],nb_re_per_symbol<<2);
